...
impl Progress {
    ...
    pub fn get_progress(
        ...
        use crate::schema::progress::dsl::{
            challenge_id as challenge_id_col, repository_id as repository_id_col,
            user_id as user_id_col,
        };

        match (id, user_id, challenge_id, repository_id) {
            (None, None, None, Some(repo_id)) => {
                let progress = progress_table
                    .filter(repository_id_col.eq(repo_id))
            ...
            (None, None, Some(challenge_id), None) => {
                let progress = progress_table
